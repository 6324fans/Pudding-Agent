<p align="center">
  <img src="assets/icon.png" width="120" height="120" alt="Pudding-Agent">
</p>

<h1 align="center">Pudding-Agent</h1>
...
## What is Pudding-Agent

Pudding-Agent is a desktop AI coding assistant that connects to your models and gives them full read/write/execute access to your codebase through **30+ built-in tools**. It works with Claude, GPT, Gemini, Ollama, or any OpenAI-compatible endpoint — speaking three protocols natively (`Anthropic Messages`, `OpenAI Chat Completions`, `OpenAI Responses`).
...
```bash
xattr -cr "/Applications/Pudding-Agent.app"
```
...
Pudding-Agent ships with no built-in models — you bring your own API keys. Open **Settings** (`⌘,` / `Ctrl+,`) and go to the **Models** tab.
...
Skills are reusable prompt templates that become slash commands. Drop a markdown file into `.puddingagent/skills/` and it's auto-discovered.

**Example** — create `.puddingagent/skills/code-review.md`:
...
Skills can live in `~/.puddingagent/skills/` (global) or `<project>/.puddingagent/skills/` (project-specific). Project skills override global ones with the same name.
...
Press `Shift+Tab` or click the plan toggle in the composer toolbar. Plans are saved as markdown in `.puddingagent/plans/`.

### 🔌 MCP Servers

Connect external tools via [Model Context Protocol](https://modelcontextprotocol.io). Configure servers in `.puddingagent/mcp-servers.json` (project) or `~/.puddingagent/mcp-servers.json` (global). Project configs override global ones for servers with the same name.
...
Run shell commands before or after tool calls to enforce policies or trigger side effects. Configure in `.puddingagent/hooks.json`.
...
The AI remembers your preferences, project context, and past decisions across sessions. Memory is automatically extracted during context compaction and stored as markdown files in `~/.puddingagent/projects/<project-path>/memory/`.
...
**`PUDDINGAGENT.md`** — place at the project root for project-specific guidance, or at `~/.puddingagent/PUDDINGAGENT.md` for global instructions. This is injected into the system prompt.

**`.puddingagent/rules/*.md`** — modular rule files loaded alongside `PUDDINGAGENT.md`. Use for separating concerns (e.g., `testing.md`, `style.md`, `deployment.md`).

Example `PUDDINGAGENT.md`:
...
All config files live under `.puddingagent/` in your project (project scope) or `~/.puddingagent/` (global scope). Project settings override global ones.

```
.puddingagent/
├── PUDDINGAGENT.md          # Project instructions (loaded in system prompt)
...
Pudding-Agent is source-available software for non-commercial use only. Commercial use is prohibited. See [LICENSE](./LICENSE).
