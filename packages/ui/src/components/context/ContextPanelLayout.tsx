import type { ConstraintObservabilitySnapshot, ContextInspectPayload } from '@puddingagent/core'
        ...
        <div className="flex min-w-0 items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex min-w-0 items-center gap-2">
              ...
              <div className="truncate font-mono text-[11px] font-semibold uppercase text-[var(--text)]">Pudding 上下文引擎</div>
            </div>
...
function contextTabs(inspect: ContextInspectPayload | null, memoryReview: ContextMemoryReview | null, providerHealth: ContextProviderHealth | null, constraint: ConstraintObservabilitySnapshot | null) {
  const factCount = dedupedMemoryFactCount(inspect, memoryReview)
...
function dedupedMemoryFactCount(inspect: ContextInspectPayload | null, memoryReview: ContextMemoryReview | null): number {
  const ids = new Set<string>()
  for (const fact of memoryReview?.accepted?.results ?? []) ids.add(fact.id)
  for (const fact of inspect?.acceptedProjectFacts ?? []) ids.add(fact.id)
  return ids.size
}
