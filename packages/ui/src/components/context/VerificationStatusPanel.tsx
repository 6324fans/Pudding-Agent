...
export function VerificationStatusPanel({ verification, onReload }: {
  ...
  if (verification.loading) return <PanelState title="正在读取验证状态" message="Verification ledger" />
  if (verification.error) return <PanelState title="验证状态暂不可用" message={verification.error} />
  if (!verification.data) return <PanelState title="暂无验证状态" message="Verification ledger" />
  ...
  return (
    <section className="space-y-3">
      <PanelHeader title="Verification" actionLabel="刷新" onAction={onReload} />
      {snapshot.status === 'unavailable' && (
        <PanelState title="Ledger 暂不可用" message={snapshot.diagnostics[0] ?? 'Verification ledger unavailable'} />
      )}

      <div className="grid grid-cols-3 gap-2">
        <Metric label="待验证" value={pendingFiles.length} />
        <Metric label="命令" value={snapshot.commands.length} />
        <Metric label="Policy" value={snapshot.policyEvents.length} />
      </div>
            ...
            <article key={file.filePath} className="min-w-0 rounded-[6px] border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-[10px] text-[var(--muted)]">tool {file.changedByToolUseId || 'unknown'}</div>
                </div>
                <StatusPill tone={file.status === 'failed' ? 'bad' : 'warn'}>{file.status}</StatusPill>
              </div>
          ...
          <article key={requirement.id} className="min-w-0 rounded-[6px] border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-[10px] uppercase tracking-[0.08em] text-[var(--muted)]">{requirement.kind}</div>
              </div>
              <StatusPill tone={requirement.status === 'failed' ? 'bad' : 'warn'}>{requirement.status}</StatusPill>
            </div>
            ...
            <article key={command.toolUseId || `${command.command}-${command.createdAt}`} className="min-w-0 rounded-[6px] border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-[10px] uppercase tracking-[0.08em] text-[var(--muted)]">{command.kind}</div>
                </div>
                <StatusPill tone={command.status === 'passed' ? 'good' : 'bad'}>{command.status}</StatusPill>
              </div>
            </article>
          ))}
        </div>
...
function PolicyEvents({ events }: { events: VerificationInspectSnapshot['policyEvents'] }) {
  ...
  return (
    <section className="space-y-2">
      <PanelHeader title="Policy events" />
      <div className="space-y-1.5">
          ...
          <article key={event.id} className="min-w-0 rounded-[6px] border border-[var(--border)] bg-[var(--surface-2)] px-3 py-2">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 text-[10px] uppercase tracking-[0.08em] text-[var(--muted)]">{event.source} · {event.phase}</div>
              </div>
              <StatusPill tone={event.decision === 'block' ? 'bad' : 'warn'}>{event.decision}</StatusPill>
            </div>
