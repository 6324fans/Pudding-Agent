...
type SectionId = 'session' | 'usage' | 'context' | 'tasks' | 'queue' | 'files' | 'team'
...
const INSPECTOR_WIDTH_KEY = 'puddingagent.inspector.width'
const MIN_WIDTH = 280
const MAX_WIDTH = 700
...
export function Inspector() {
  ...
  useEffect(() => {
    ...
    if (activeSection === 'context') {
      loadProjectContext(activeSessionId)
    }
  }, [activeSessionId, activeSection, loadProjectContext])

  useEffect(() => {
    if (activeSessionId && activeSection === 'context' && wasStreamingRef.current && !isStreaming) {
      loadProjectContext(activeSessionId)
    }
    wasStreamingRef.current = isStreaming
  }, [activeSessionId, activeSection, isStreaming, loadProjectContext])
  ...
  const contextBadge = contextWarningCount > 0 ? contextWarningCount : null
  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 flex items-center justify-between px-3 py-2 border-b border-[var(--border)]">
        <span className="text-[12px] font-medium text-[var(--text)]">检查器</span>
        <button
      ...
      {activeSection === 'team' ? (
        <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
          {effectiveTeamId && activeSessionId ? (
            ...
            <div className="p-3">
              <p className="text-[12px] text-[var(--muted)]">暂无活跃团队。可通过团队工具创建。</p>
            </div>
          )}
        </div>
      ) : (
...
function SessionSection({ sessionId }: { sessionId: string | null }) {
  return (
    <div>
      <SectionHeader>会话</SectionHeader>
...
function UsageSection({ usage }: {
  ...
  if (!usage) {
    return (
      <div>
        <SectionHeader>用量</SectionHeader>
        <p className="text-[12px] text-[var(--muted)]">暂无用量数据</p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      {backgroundTasks.length > 0 && (
        <div>
          <SectionHeader>后台任务（{runningBg.length} 个运行中）</SectionHeader>
      ...
      {tasks.length > 0 && (
        <div>
          <SectionHeader>任务（{tasks.filter(t => t.status === 'completed').length}/{tasks.length}）</SectionHeader>
          <div className="space-y-1.5">
            {tasks.map((task) => (
      ...
      {backgroundTasks.length === 0 && tasks.length === 0 && (
        <div>
          <SectionHeader>任务</SectionHeader>
          <p className="text-[12px] text-[var(--muted)]">暂无任务</p>
        </div>
      )}
    </div>
  )
...
function QueueSection({ queue, removeFromQueue }: { queue: string[]; removeFromQueue: (i: number) => void }) {
  if (queue.length === 0) {
    return (
      <div>
        <SectionHeader>队列</SectionHeader>
        <p className="text-[12px] text-[var(--muted)]">队列为空</p>
      </div>
    )
  }
...
function FilesSection({ files }: { files: FileChange[] }) {
  if (files.length === 0) {
    return (
      <div>
        <SectionHeader>文件变更</SectionHeader>
        <p className="text-[12px] text-[var(--muted)]">暂无文件变更</p>
      </div>
    )
  }
