...
const STATUS_LABELS: Record<string, string> = {
  ...
  planning: '规划中',
  assigning: '分配中',
  waiting_for_members: '等待成员',
  reviewing_results: '审查结果',
  handling_intervention: '处理中断',
  synthesizing: '汇总中',
  todo: '待办',
  assigned: '已分配',
  cancelled: '已取消',
  ...
  if (!team) {
    return (
      <div className="flex flex-col h-full min-h-0 bg-[var(--panel)]">
        <Header title={`团队 ${taskId.slice(0, 8)}`} onClose={onClose} />
          ...
          <div className="p-3 space-y-4">
            ...
            <Section title="项目经理">
              {team.manager ? (
                ...
                <div className="text-[11px] text-[var(--muted)]">初始化中…</div>
              )}
            </Section>

            <Section title={`成员 (${team.members?.length ?? 0})`}>
            ...
            <Section title={`任务 (${team.taskStats?.completed ?? 0}/${team.taskStats?.total ?? 0})`}>
            ...
            {conversation.length > 0 && (
              <Section title={`对话 (${conversation.length})`}>
                <ul className="space-y-1.5">
          ...
          <div className="h-full flex flex-col">
            <div className="flex-1 min-h-0 overflow-y-auto p-3">
              <pre className="text-[11px] leading-[1.55] font-mono text-[var(--muted)] whitespace-pre-wrap break-all m-0">
                {events.length === 0 ? '(暂无事件)' : events.slice(-200).join('\n')}
              </pre>
      ...
      <div className="flex-shrink-0 border-t border-[var(--border)] p-3 space-y-2">
        {isFinished && (
          <div className="text-[10px] text-[var(--muted)] italic px-1">
            团队状态为「{STATUS_LABELS[team.status] || team.status}」，消息已禁用。
          </div>
        )}
        ...
        <div className="flex items-center gap-1.5">
          ...
          <input
            type="text"
            value={message}
            placeholder={isFinished ? '团队已结束' : '发送给项目经理…'}
            disabled={isFinished}
...
function ConversationBubble({ entry }: { entry: TeamConversationEntry }) {
  ...
  const fromLabel = isSent ? '你' : entry.from === 'pm' ? '项目经理' : entry.from.replace('member:', '@')
...
function StatsRow({ team }: { team: any }) {
  ...
  const stats = [
    { label: '成员', value: team.members?.length ?? 0 },
    { label: '完成', value: ts.completed ?? 0, color: 'var(--good)' },
    { label: '运行', value: ts.running ?? 0, color: 'var(--accent)' },
    { label: '阻塞', value: ts.blocked ?? 0, color: 'var(--warning, orange)' },
    { label: '失败', value: ts.failed ?? 0, color: 'var(--bad)' },
  ].filter((s) => s.value > 0)
    ...
    >
      <div className="flex items-center gap-2 px-3 py-2">
        ...
        <span className="text-[10px] text-[var(--muted)] flex-shrink-0">
          {member.toolCount > 0 ? `${member.toolCount} 个工具` : ''}
        </span>
            ...
            {task ? (
              ...
              <div className="text-[11px] text-[var(--muted)] italic">
                {member.currentTaskId
                  ? `(当前快照中未找到任务 ${member.currentTaskId})`
                  : '(空闲，未处理任务)'}
              </div>
            )}
          </ModalSection>

          <ModalSection title={`最近事件 (${memberEvents.length})`}>
            {memberEvents.length === 0 ? (
              <div className="text-[11px] text-[var(--muted)] italic">(该成员暂无事件)</div>
            ) : (
