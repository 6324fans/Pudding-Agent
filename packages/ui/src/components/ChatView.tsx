...
import type { Message } from '@puddingagent/core'

const INIT_PROMPT = `Read the project structure in the current working directory and generate a PUDDINGAGENT.md file in the project root. This file provides guidance to PUDDINGAGENT when working with code in this repository.
...
Keep it concise and practical. Write the file using the Write tool to the project root as PUDDINGAGENT.md. If one already exists, read it first and update it with any new information.`
...
export function ChatView({ onOpenMcp }: ChatViewProps) {
  const { messages, streamingText, thinkingText, isStreaming, aborting, isThinking, toolEvents, sendMessage, abort, error, retry, dismissError, usage } = useSession()
  ...
  const [permissionMode, setPermissionMode] = useState(() => {
    return localStorage.getItem('puddingagent-permission-mode') || 'standard'
  })
  const [effort, setEffort] = useState<'off' | 'low' | 'medium' | 'high' | 'xhigh' | 'max'>(() => {
    const stored = localStorage.getItem('puddingagent-effort')
  ...
  const handlePermissionChange = useCallback((mode: string) => {
    ...
    localStorage.setItem('puddingagent-permission-mode', mode)
  ...
  const handleEffortChange = useCallback((next: 'off' | 'low' | 'medium' | 'high' | 'xhigh' | 'max') => {
    const api = (window as any).electronAPI
    setEffort(next)
    localStorage.setItem('puddingagent-effort', next)
  ...
  const handleSlashCommand = (command: string) => {
    ...
    switch (command) {
      ...
      case '/status':
        showToast(`会话: ${activeSessionId?.slice(0, 8)} | 消息: ${messages.length} | 模型: ${activeModel?.model.name || '-'} | 推理: ${effort}`)
        break
      case '/commit':
        if (activeSessionId && api?.invoke) {
          api.invoke('file:get-changes', { sessionId: activeSessionId }).then((files: any[]) => {
            if (!files || files.length === 0) {
              showToast('当前会话没有文件变更')
            } else {
      ...
      <div className={`relative flex flex-col overflow-hidden ${showRightPanel ? 'w-[60%]' : 'w-full'} transition-all`}>
            ...
            {isStreaming && !streamingText && !isThinking && toolEvents.length === 0 && !isLastTurnActive && (
              <div className="mb-3 border border-[var(--border)] rounded-[8px]">
                <div className="flex items-center gap-2 px-3 py-2 text-[10px] uppercase tracking-[0.1em]">
                  ...
                  <span className="text-[var(--text)]">处理中...</span>
                </div>
              </div>
            )}
        ...
        {planMode && (
          <div className="border-t border-[var(--plan)] px-4 py-1.5 flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase tracking-[0.1em] text-[var(--plan)]">规划模式</span>
        ...
        <Composer
          ...
          onModelClick={openModelSettings}
