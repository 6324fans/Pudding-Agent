...
export function AppLauncher({ cwd }: Props) {
  ...
  const openApp = async (appId: string) => {
    ...
    const result = await window.electronAPI?.appsOpen(appId, cwd) as { success: boolean; error?: string } | undefined
    if (result?.success) {
      showToast(`已打开 ${app?.name || '应用'}，正在连接 IDE`, 'info')
    } else {
      showToast(`打开 ${app?.name || '应用'} 失败：${result?.error || '未知错误'}`, 'error', 4000)
    }
    setOpen(false)
  }
  ...
  return (
    <div className="relative flex items-center" ref={ref}>
      <button
        ...
        aria-label={`在 ${defaultApp.name} 中打开`}
      >
