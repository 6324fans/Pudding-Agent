...
function taskDetail(toolName: string, input: Record<string, unknown>): string {
  if (toolName === 'todo_write' && Array.isArray(input.todos)) return `${input.todos.length} todos`
  if (toolName === 'task_create') return stringValue(input.subject) || 'create task'
  if (toolName === 'task_update') return `${stringValue(input.taskId || input.task_id) || 'task'} · ${stringValue(input.status) || 'update'}`
  if (toolName === 'task_get' || toolName === 'task_stop' || toolName === 'task_output') return stringValue(input.taskId || input.task_id) || 'task'
  if (toolName === 'save_memory') return `${stringValue(input.name)} · ${stringValue(input.type)}`
  if (toolName === 'ask_user') return stringValue(input.question) || 'waiting for user'
  if (toolName === 'Team') return stringValue(input.objective) || 'team objective'
  if (toolName === 'background_status' || toolName === 'background_events' || toolName === 'background_send') return stringValue(input.task_id) || 'background task'
    ...
    return (
      <div className="tool-list">
        {input.todos.map((todo, index) => {
          const item = todo as { content?: unknown; subject?: unknown; description?: unknown; status?: unknown }
          const description = stringValue(item.description)
          return (
            <div key={index}>
              <span>{index + 1}</span>
              <strong>{stringValue(item.content) || stringValue(item.subject) || 'Untitled task'}</strong>
              {item.status && <em>{stringValue(item.status)}</em>}
  ...
  return (
    ...
    >
      ...
      {content ? (
        ...
        <div className="tool-empty-state">处理中...</div>
      ) : (
