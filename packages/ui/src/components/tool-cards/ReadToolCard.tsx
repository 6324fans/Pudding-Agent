...
export function ReadToolCard({ event, input, result }: ToolCardRouterProps) {
  ...
  return (
    <ToolCardShell
      ...
      actions={status === 'done' ? (
        <button onClick={(e) => { e.stopPropagation(); copyToClipboard(filePath) }} className="p-1 rounded-[4px] text-[var(--muted)] hover:text-[var(--text)] hover:bg-[var(--surface-3)] transition-colors" aria-label="复制路径">
          <IconCopy size={12} />
        </button>
      ) : undefined}
    >
