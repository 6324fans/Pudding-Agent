...
export function NotebookEditToolCard({ event, input, result, name }: ToolCardRouterProps) {
  ...
  const toolName = event?.toolName || name || 'notebook_edit'
  ...
  return (
    <ToolCardShell
      ...
      actions={newSource ? (
        <ToolCopyButton text={newSource} label="源码" title="复制源码" iconOnly />
      ) : undefined}
    >
      ...
      {!isError && editMode === 'DELETE' && (
        <div className="tool-empty-state">该单元格将被删除。</div>
      )}
      ...
      {!isError && editMode !== 'DELETE' && newSource && (
        <div className="tool-code-frame max-h-[240px] overflow-auto text-[11px]" style={{ fontFamily: 'var(--font-mono)' }}>
          ...
          {hasMore && (
            ...
            >
              显示剩余 {sourceLines.length - displayLines.length} 行
            </button>
          )}
        </div>
      )}
