    ...
    <div onClick={handleClick} className="cursor-pointer">
      ...
      >
        {status === 'running' && recentTools.length > 0 && (
          <div className="pudding-agent-timeline text-[12px] text-[var(--muted)] mb-2" style={{ fontFamily: 'var(--font-mono)' }}>
            {recentTools.map((te, i) => (
              <div key={i} className="pudding-agent-timeline-row">
                <span className="pudding-agent-timeline-dot" data-status={te.status} />
        ...
        {status === 'running' && toolCount > 0 && (
          <div className="text-[10px] text-[var(--muted)] mt-1">已执行 {toolCount} 个工具</div>
        )}
        {status !== 'running' && prompt && (
          <div className="mb-2">
            <div className="text-[10px] uppercase tracking-[0.1em] text-[var(--muted)] mb-1">输入</div>
