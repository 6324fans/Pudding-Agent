...
export class GitService {
  ...
  async stash(cwd: string): Promise<{ success: boolean; error?: string }> {
    try {
      await this.git(['stash', 'push', '-m', 'puddingagent-auto-stash'], cwd)
      return { success: true }
    } catch (err: any) {
      return { success: false, error: err.stderr || err.message }
    }
  ...
  async stashPop(cwd: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      const idx = lines.findIndex(l => l.includes('puddingagent-auto-stash'))
      if (idx === -1) {
  ...
  async hasStash(cwd: string): Promise<boolean> {
    ...
    return output.includes('puddingagent-auto-stash')
  }
