...
export class AppLauncher {
  ...
  detect(): DetectedApp[] {
    ...
    if (process.platform === 'darwin') {
      this.cache = APP_DEFS.filter((appDef) => {
        ...
        return Boolean(findDarwinAppName(appDef))
  ...
  open(appId: string, cwd: string): Promise<{ success: boolean; error?: string }> {
    ...
    const def = this.getOpenCommand(appDef, cwd)
    if (!def) return Promise.resolve({ success: false, error: `${appDef.name} not found` })
  ...
  private getOpenCommand(appDef: AppDef, cwd: string): CmdDef | null {
    if (process.platform === 'darwin') {
      if (appDef.id === 'finder') return appDef.getCmd(cwd)

      const appName = findDarwinAppName(appDef)
      if (!appName) {
        if (appDef.alwaysAvailable) return appDef.getCmd(cwd)
        return null
      }
      return { cmd: 'open', args: ['-a', appName, cwd] }
    }

    if (process.platform === 'win32' && appDef.winGetCmd) {
      return appDef.winGetCmd(cwd)
    }

    return appDef.getCmd(cwd)
  }
}

function findDarwinAppName(appDef: AppDef): string | null {
  for (const bundle of appDef.bundleNames) {
    for (const dir of DARWIN_APP_DIRS) {
      if (existsSync(path.join(dir, bundle))) {
        return bundle.replace(/\.app$/i, '')
      }
    }
  }
  return null
