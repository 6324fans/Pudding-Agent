...
describe('plan agent restrictions', () => {
  it('allows writing to .puddingagent/plans/', () => {
    expect(isWriteAllowedForPlanAgent('/project/.puddingagent/plans/my-plan.md', '/project')).toBe(true)
  })

  it('rejects writing outside .puddingagent/plans/', () => {
    expect(isWriteAllowedForPlanAgent('/project/src/index.ts', '/project')).toBe(false)
  })

  it('rejects relative path escape', () => {
    expect(isWriteAllowedForPlanAgent('/project/.puddingagent/plans/../../etc/passwd', '/project')).toBe(false)
  })
})
