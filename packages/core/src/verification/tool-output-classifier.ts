...
function classifyCommandSegment(segment: string): VerificationKind | undefined {
  const normalized = stripCorepackPrefix(stripShellComment(segment).trim().toLowerCase())
  ...
  if (/^(gradle|gradlew|\.\/gradlew)\s+test\b/.test(normalized)) return 'test'
...
function stripCorepackPrefix(segment: string): string {
  return segment.replace(/^corepack\s+/, '')
}
