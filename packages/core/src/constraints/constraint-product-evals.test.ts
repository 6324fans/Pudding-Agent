...
describe('Pudding Agent Constraint Engine Phase 3 product evals', () => {
  const tmpDir = path.join(os.tmpdir(), 'pudding-constraint-phase3-eval')
  ...
  it('blocks unread mutation, records policy event, then records pending verification after read and edit', async () => {
    ...
    expect(blocked.content).toContain('Blocked by Pudding Agent Constraint Engine')
