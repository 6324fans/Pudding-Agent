...
describe('ConstraintPolicyRuntime', () => {
  const tmpDir = path.join(os.tmpdir(), 'pudding-policy-runtime-test')
  ...
  it('marks pending changed files verified after a successful verification command', () => {
    ...
    runtime.postToolUse({
      toolName: 'Bash',
      toolUseId: 'bash_1',
      input: { command: 'pnpm --filter @puddingagent/core build' },
      cwd: tmpDir,
      fileReadState,
      result: {
        content: 'build ok',
        metadata: { command: { shell: 'bash', command: 'pnpm --filter @puddingagent/core build', exitCode: 0 } },
      },
    })
