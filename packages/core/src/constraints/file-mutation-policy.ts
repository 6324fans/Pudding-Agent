...
const MUTATION_TOOLS = new Set(['Edit', 'MultiEdit', 'Write', 'file_edit', 'multi_edit', 'file_write'])
...
export function evaluateFileMutationPolicy(policyInput: FileMutationPolicyInput): FileMutationPolicyDecision {
  ...
  if (isWriteTool(policyInput.toolName) && !existsSync(resolvedFilePath)) {
    return allow()
  }

  if (isEditTool(policyInput.toolName)) {
    return checkFreshRead(policyInput.fileReadState, resolvedFilePath, getRequiredText(policyInput.input, 'old_string'))
  }

  if (isMultiEditTool(policyInput.toolName)) {
  ...
  if (isWriteTool(policyInput.toolName)) {
...
function isEditTool(toolName: string): boolean {
  return toolName === 'Edit' || toolName === 'file_edit'
}

function isMultiEditTool(toolName: string): boolean {
  return toolName === 'MultiEdit' || toolName === 'multi_edit'
}

function isWriteTool(toolName: string): boolean {
  return toolName === 'Write' || toolName === 'file_write'
}
