...
describe('context-engine: snapshot persistence', () => {
  it('writes a snapshot to .puddingagent/context-engine/index.json after indexing', async () => {
    const tmp = setup()
    ...
    expect(existsSync(path.join(tmp, '.puddingagent', 'context-engine', 'index.json'))).toBe(true)
  })
