...
// under .puddingagent/context-engine/index.json so a restart can load instantly and
...
function snapshotDir(cwd: string): string {
  return path.join(cwd, '.puddingagent', 'context-engine')
}
...
// The whole .puddingagent/ directory holds tool-generated artifacts (index cache,
// plans, rules, etc.), so ignore it wholesale rather than just the snapshot dir.
const IGNORE_ENTRY = '.puddingagent/'

/**
 * Ensure the .puddingagent/ directory is gitignored so opening the app on a user's
