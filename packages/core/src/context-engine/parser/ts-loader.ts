...
//    bundle, so we honor PUDDING_TREE_SITTER_WASM_DIR (runtime) and
//    PUDDING_GRAMMAR_WASM_DIR (grammars) as overrides.
 ...
 *  - esbuild CJS bundle (electron): use __filename as the require base
 * We avoid relying only on import.meta.url because esbuild rewrites it to an
 * empty object in the CJS bundle.
 */
function resolvePackageFile(request: string): string | null {
  // 1. Plain CJS / esbuild CJS bundle.
  try {
