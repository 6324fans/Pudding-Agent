  ...
  return [
    {
      ...
      name: 'Relevant file recall uses the existing Pudding tools surface',
      run: async () => {
        ...
        try {
          ...
          const search = await tools.PuddingSearch.execute({ query: 'computeContextBudget' }, toolContext)
          assert.notEqual(search.isError, true)
          assert.match(search.content, /src\/context-budget\.ts/)
          const context = await tools.PuddingContext.execute({ task: 'how does planContext use computeContextBudget', maxNodes: 8, includeCode: true }, toolContext)
    ...
    },
    {
      ...
      run: async () => {
        const fixture = makeProjectFixture()
        try {
          writeFileSync(path.join(fixture.cwd, 'PUDDINGAGENT.md'), 'Instruction: do not duplicate.')
          writeFileSync(path.join(fixture.cwd, 'README.md'), 'Readable project overview.')
          const report = await buildContextBundle(makeEvalRequest({
            cwd: fixture.cwd,
            carriedContext: {
              projectInstructionRefs: ['PUDDINGAGENT.md'],
    ...
    {
      id: 'regression-pudding-tools',
      category: 'regression',
      name: 'Existing Pudding* code tools still work with the context eval fixture',
      run: async () => {
        const fixture = makeCodeFixture()
        try {
          const tools = Object.fromEntries(createContextEngineTools().map((tool) => [tool.definition.name, tool]))
          const context = await makeIndexedEvalToolContext(fixture.cwd)
          const search = await tools.PuddingSearch.execute({ query: 'planContext' }, context)
          const callers = await tools.PuddingCallers.execute({ symbol: 'computeContextBudget' }, context)
          const node = await tools.PuddingNode.execute({ symbol: 'planContext', includeCode: true }, context)
          const trace = await tools.PuddingTrace.execute({ from: 'planContext', to: 'computeContextBudget' }, context)
          const files = await tools.PuddingFiles.execute({ path: 'src' }, context)
    ...
    },
    {
      ...
      run: () => {
        const prompt = '<pudding-context-engine bundle="ctx_eval"><section kind="runtime_state" confidence="0.9" freshness="live" source="Eval">Runtime evidence</section></pudding-context-engine>'
        ...
        assert.match(String(chatFormatted[0].content), /<pudding-context-engine bundle="ctx_eval">/)
...
function makeCodeFixture(): { cwd: string; cleanup: () => void } {
  const cwd = mkdtempSync(path.join(tmpdir(), 'pudding-context-evals-'))
...
function makeStoreFixture(): { dbPath: string; cleanup: () => void } {
  const cwd = mkdtempSync(path.join(tmpdir(), 'pudding-context-store-eval-'))
  return { dbPath: path.join(cwd, 'pudding-context.db'), cleanup: () => rmSync(cwd, { recursive: true, force: true }) }
}

function makeProjectFixture(): { cwd: string; cleanup: () => void } {
  const cwd = mkdtempSync(path.join(tmpdir(), 'pudding-context-product-eval-'))
  return { cwd, cleanup: () => rmSync(cwd, { recursive: true, force: true }) }
}
