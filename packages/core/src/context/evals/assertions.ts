...
export function makeEvalMessage(overrides: Partial<Message> = {}): Message {
  return {
    id: 'msg_eval_user',
    role: 'user',
    content: [{ type: 'text', text: 'Remember that Pudding Context Engine production facts require citations.' }],
    timestamp: 1,
    ...overrides,
  }
}
...
export function makeEvalFact(overrides: Partial<ContextFact> = {}): ContextFact {
  return {
    ...
    kind: 'workflow_rule',
    scope: 'project',
    content: 'Durable Pudding Context Engine facts require citations.',
...
export function assertContextEvalReportPassed(report: ContextEvalReport): void {
  ...
  const failures = report.cases
    ...
    .join('\n')
  throw new Error(`Pudding Context Engine Gate F evals failed:\n${failures}`)
}
