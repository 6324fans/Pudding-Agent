...
export async function buildContextBundle(request: ContextRequest, options: BuildContextBundleOptions): Promise<BuildContextBundleResult> {
  ...
  if (options.injectionEnabled === false) {
    const bundle = emptyBundle(request, createId(), now(), [diagnostic('ContextOrchestrator', 'info', 'Pudding Context Engine context injection disabled; returning empty bundle.', now())])
...
function agentContractSections(plan: ContextPlan, _createdAt: number, modelProfile?: ContextRequest['modelProfile']): ContextSection[] {
  ...
  return [{
    ...
    confidence: 1,
    freshness: 'live',
    sourceProvider: 'PuddingAgentConstraintEngine',
