...
describe('WorkflowSignalProvider', () => {
  ...
  it('collects release, build, test, and package signals from bounded workflow files', async () => {
    ...
    writeProjectFile(cwd, '.github/workflows/release.yml', [
      ...
      '        run: |',
      '          pnpm package',
      '          pnpm --filter @puddingagent/core test',
    ].join('\n'))
    writeProjectFile(cwd, 'package.json', JSON.stringify({
      scripts: {
        build: 'pnpm -r build',
        test: 'pnpm --filter @puddingagent/core test',
    ...
    expect(content).toContain('pnpm --filter @puddingagent/core test')
...
function tempProject(): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'pudding-workflow-provider-'))
  dirs.push(dir)
  return dir
}
