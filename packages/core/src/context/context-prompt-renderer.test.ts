...
describe('Pudding Context prompt renderer', () => {
  it('does not inject legacy file-based MEMORY.md instructions or index into the system prompt', async () => {
    const cwd = mkdtempSync(path.join(tmpdir(), 'pudding-context-prompt-'))
    tmpDirs.push(cwd)
  ...
  it('renders a protocol-neutral Pudding Context Engine XML block with required section attributes and compact citations', () => {
    const rendered = renderContextBundle(makeBundle())

    expect(rendered).toMatch(/^<pudding-context-engine bundle="ctx_[0-9a-f]{16}">/)
    ...
    expect(rendered).toContain('</pudding-context-engine>')
