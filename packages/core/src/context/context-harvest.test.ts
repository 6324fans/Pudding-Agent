...
function projectProfileEnvelope(projectPurpose: string, citationId: string): DistillerEnvelope {
  return {
    ...
    payload: {
      ...
      commands: [{ name: 'core build', command: 'pnpm --filter @puddingagent/core build', purpose: 'Compile core package.' }],
...
function makeDbPath(): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'pudding-context-harvest-'))
  tmpDirs.push(dir)
  return path.join(dir, 'pudding-context.db')
}
