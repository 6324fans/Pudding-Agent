...
describe('ContextRetriever', () => {
  it('retrieves old relevant workflow memory ahead of newer irrelevant facts without a hidden candidate cap', async () => {
    const facts = [
      ...
      fact({
        id: 'old_release_flow',
        kind: 'workflow_rule',
        content: 'PUDDINGAGENT 发布流程：修改 packages/electron/package.json version，提交 bump commit，打 tag，然后 push tag 触发 release workflow。',
        updatedAt: 1,
        confidence: 1,
      }),
    ]
  ...
  it('honors an explicit result limit when query is empty', async () => {
    const store = makeStore([
      ...
      fact({ id: 'rule_2', kind: 'project_convention', content: 'Use Pudding Context Engine naming.', updatedAt: 20 }),
