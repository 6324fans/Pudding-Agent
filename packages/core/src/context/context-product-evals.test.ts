...
describe('Pudding Context Engine product evals', () => {
  ...
  it('keeps a large relevant project primer when no explicit cap is configured', async () => {
    const primer = makeEvalSection({
      ...
      content: 'PUDDINGAGENT 项目背景 '.repeat(3_000),
      tokenEstimate: 15_000,
      priority: 100,
      sourceProvider: 'ProjectSignalProvider',
    })

    const result = await buildContextBundle(makeEvalRequest({ tokenBudget: undefined, userMessage: '解释 PUDDINGAGENT 项目背景' }), {
    ...
    expect(result.renderedPrompt).toContain('PUDDINGAGENT 项目背景')
  ...
  it('does not inject project instruction files already carried by the system prompt', async () => {
    const cwd = tempProject()
    writeFileSync(path.join(cwd, 'PUDDINGAGENT.md'), 'Instruction: prefer pnpm.')
  ...
  it('retrieves a relevant old memory without injecting hundreds of recent memories', async () => {
    ...
    await store.saveRawEvidence({
      ...
      sourceProvider: 'ProductEval',
      kind: 'message',
      content: '用户要求记住 PUDDINGAGENT 发布流程：修改 package version，commit bump，tag vX.Y.Z，push tag 触发 release workflow。',
    ...
    await store.saveFact({
      ...
      kind: 'workflow_rule',
      scope: 'project',
      content: 'PUDDINGAGENT 发布流程：修改 package version，commit bump，tag vX.Y.Z，push tag 触发 release workflow。',
    ...
    expect(report.renderedPrompt).toContain('PUDDINGAGENT 发布流程')
  ...
  it('preserves project documentation content beyond the first three non-empty lines', async () => {
    const cwd = tempProject()
    writeFileSync(path.join(cwd, 'PUDDINGAGENT.md'), [
      '# PUDDINGAGENT',
      '第一行简介。',
      '第二行简介。',
      '第三行简介。',
  ...
  it('answers release-flow questions from cited workflow files automatically', async () => {
    ...
    writeProjectFile(cwd, '.github/workflows/release.yml', [
      ...
      '        run: |',
      '          pnpm package',
      '          pnpm --filter @puddingagent/vscode-extension package',
    ].join('\n'))
    ...
    expect(result.renderedPrompt).toContain('pnpm --filter @puddingagent/vscode-extension package')
  ...
  it('renders repo map context only when planning or repo-map evidence is requested, and keeps it compact', async () => {
    ...
    const puddingFiles = createContextEngineTools().find((tool) => tool.definition.name === 'PuddingFiles')
    expect(puddingFiles).toBeDefined()
    const filesResult = await puddingFiles!.execute({}, { cwd, turnIndex: 0, contextEngine: engine } as any)
  ...
  it('Phase 8 eval: repo wiki survives generation, retrieval, and file-hash invalidation', async () => {
    ...
    writeProjectFile(cwd, 'README.md', '# PUDDINGAGENT\n\nContext orchestration runtime.')
...
function tempProject(): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'pudding-context-product-eval-'))
  dirs.push(dir)
  return dir
}
