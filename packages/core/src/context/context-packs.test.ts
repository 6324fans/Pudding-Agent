...
describe('Pudding Context actor-aware context packs', () => {
  ...
  it('ranks subagent packs toward parent objective and project/code facts instead of unrelated recent chat', async () => {
    ...
    const actorProfile = subAgentProfile({
      ...
      subSessionId: 'sub_1',
      parentObjective: 'Fix Pudding Context Engine injection',
...
function sharedFactPool(): ContextFact[] {
  return [
    ...
    fact({
      ...
      content: 'Pudding Context Engine injection entrypoint is Session.injectContextForRunLoop in packages/core/src/session.ts.',
