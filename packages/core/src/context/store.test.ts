...
describe('Pudding Context Store persistence', () => {
  it('opens the default store under the current project .puddingagent context-engine root', async () => {
    ...
    try {
      ...
      expect(info.value.dbPath).toBe(path.join(process.cwd(), '.puddingagent', 'context-engine', 'context.db'))
  ...
  it('merges duplicate canonical facts while preserving citations and confidence', async () => {
    ...
    await expectOk(store.saveFact(makeFact({
      ...
      content: 'Run pnpm --filter @puddingagent/core build before release.',
      confidence: 0.8,
  ...
  it('falls back to diagnostics instead of throwing when the store cannot open', async () => {
    const dir = makeTempDir()
    const dbPath = path.join(dir, 'pudding-context.db')
    mkdirSync(dbPath)
...
function makeTempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'pudding-context-store-'))
  tmpDirs.push(dir)
  return dir
}

function makeDbPath(): string {
  return path.join(makeTempDir(), 'pudding-context.db')
}
