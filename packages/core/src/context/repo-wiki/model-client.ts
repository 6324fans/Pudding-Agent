...
export function buildRepoWikiPrompt(request: RepoWikiModelRequest): string {
  return JSON.stringify({
    task: 'Generate a Pudding Repo Wiki as one strict JSON object.',
    schema: {
      schemaVersion: 1,
      action: 'save or skip',
...
const REPO_WIKI_SYSTEM_PROMPT = 'You generate citation-backed repository Wiki JSON for Pudding Context Engine. Use only provided evidence packet ids. Return JSON only. Do not include raw hidden reasoning.'
