...
function tmpRepo(): string {
  return path.join(tmpdir(), `pudding-repo-wiki-${Date.now()}-${Math.random().toString(36).slice(2)}`)
}
...
describe('repo wiki generation', () => {
  ...
  it('applies default repo map and packet size limits for large repositories', () => {
    ...
    const docRefs = ['README.md', 'PUDDINGAGENT.md', 'AGENTS.md', 'CLAUDE.md', 'GEMINI.md', 'CONTRIBUTING.md', 'package.json', 'pnpm-workspace.yaml', 'turbo.json']
