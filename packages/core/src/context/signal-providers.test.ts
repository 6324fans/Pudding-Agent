...
describe('context signal providers', () => {
  it('wraps the existing Pudding code engine without changing Pudding* tools', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-code-provider-'))
  ...
  it('returns fallback code health and queues background warmup when the engine is not indexed', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-code-provider-warmup-'))
  ...
  it('queues explicit code reindex after returning cached health when the engine is not indexed', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-code-provider-unindexed-'))
    const releaseIndex = deferred<void>()
    const engine = {
      isIndexed: vi.fn(() => false),
      index: vi.fn(() => releaseIndex.promise),
  ...
  it('collects project and git signals while legacy file-based memory stays retired', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-project-provider-'))
    ...
    const memoryDir = mkdtempSync(join(tmpdir(), 'pudding-memory-provider-'))
  ...
  it('does not duplicate project instruction files already loaded into the system prompt', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-project-provider-dedupe-'))
    writeFileSync(join(cwd, 'PUDDINGAGENT.md'), 'Instruction: use project style.')
    ...
    const result = await collectProjectContext(request(cwd, {
      carriedContext: {
        projectInstructionRefs: ['PUDDINGAGENT.md', 'AGENTS.md'],
  ...
  it('includes direct branch, short status, and recent log signals while preserving hot files', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-git-direct-provider-'))
    ...
    execFileSync('git', ['config', 'user.name', 'Pudding Test'], { cwd, stdio: 'ignore' })
  ...
  it('collects accepted project memories as cited memory context', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-memory-provider-store-'))
    const releaseFact: ContextFact = {
      ...
      confidence: 0.95,
      freshness: 'recent',
      sourceProvider: 'PuddingMemoryWrite',
      createdAt: 1,
      updatedAt: 1,
    }
    const store = {
  ...
  it('does not impose a default accepted-memory count cap in the provider', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-memory-provider-unlimited-store-'))
  ...
  it('keeps meaningful project docs beyond the first three non-empty lines', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-project-doc-provider-'))
    writeFileSync(join(cwd, 'PUDDINGAGENT.md'), [
      '# PUDDINGAGENT',
      '',
      '第一行简介。',
      '第二行简介。',
      ...
      '',
      '## 上下文引擎约定',
      'Pudding Context Engine 数据必须按项目持久化。',
    ].join('\n'))
  ...
  it('does not truncate project metadata files for local token budgeting', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-project-doc-full-provider-'))
    writeFileSync(join(cwd, 'PUDDINGAGENT.md'), [
      '# PUDDINGAGENT',
  ...
  it('collects conversation, runtime, and IDE signals without persisting raw thinking', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-live-provider-'))
    const liveRequest = request(cwd, {
      runtime: { toolEvents: [{ id: 'tool_1', name: 'Read', status: 'error', message: 'missing file' }, { id: 'tool_2', name: 'PuddingSearch', status: 'cancelled', message: 'sibling failed' }] },
  ...
  it('formats ToolRunner-shaped runtime events and preserves real tool error details', () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-runtime-provider-'))
  ...
  it('reports stale, failed, and rate-limited provider health with degraded diagnostics', async () => {
    const cwd = mkdtempSync(join(tmpdir(), 'pudding-degraded-provider-'))
