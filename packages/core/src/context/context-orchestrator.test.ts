...
describe('Pudding Context orchestrator', () => {
  ...
  it('builds a ranked, budgeted bundle from live providers and stored facts, then persists evidence and snapshots', async () => {
    ...
    expect(result.renderedPrompt).toMatch(/^<pudding-context-engine bundle="ctx_[0-9a-f]{16}">/)
  ...
  it('preserves actor profile metadata in the bundle and rendered prompt', async () => {
    ...
    const store = makeStore({ facts: [fact({ id: 'memory_fact', kind: 'project_convention', content: 'Use Pudding Context Engine naming.' })] })
  ...
  it('does not drop large relevant sections when no explicit token caps are configured', async () => {
    const largeProjectSection = section({
      ...
      content: 'Pudding Context Engine '.repeat(1200),
      tokenEstimate: 6000,
      priority: 90,
    ...
    expect(result.renderedPrompt).toContain('Pudding Context Engine')
  ...
  it('injects query-relevant memory instead of recent irrelevant memory through retrieval', async () => {
    const facts = [
      ...
      fact({
        id: 'release_process',
        kind: 'workflow_rule',
        content: 'PUDDINGAGENT 发布流程：bump version，commit，tag vX.Y.Z，然后 push tag 触发 release workflow。',
        updatedAt: 1,
        confidence: 1,
      }),
    ]
    ...
    expect(result.renderedPrompt).toContain('PUDDINGAGENT 发布流程')
...
function fact(overrides: Partial<ContextFact> = {}): ContextFact {
  return {
    ...
    kind: 'user_preference',
    scope: 'project',
    content: 'Use Pudding Context Engine name.',
