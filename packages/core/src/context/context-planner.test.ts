...
describe('ContextPlanner', () => {
  ...
  it('keeps runtime error chain for debug turns', () => {
    ...
    const sections = [
      ...
      section({ id: 'project_profile', kind: 'project_profile', title: 'Project', content: 'PUDDINGAGENT', confidence: 0.85 }),
    ]
  ...
  it('reports missing relevant code evidence for code_edit turns', () => {
    const plan = planContext(makeRequest({ mode: 'code_edit', userMessage: 'Fix the bug' }), [
      section({ id: 'project_profile', kind: 'project_profile', title: 'Project', content: 'PUDDINGAGENT' }),
    ])
