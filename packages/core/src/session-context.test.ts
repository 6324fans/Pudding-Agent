...
vi.mock('./config.js', () => ({
  ...
  getConfigDir: () => '/tmp/pudding-session-context-config',
}))
...
afterEach(async () => {
  ...
  rmSync('/tmp/pudding-session-context-config', { recursive: true, force: true })
  for (const dir of tmpDirs) rmSync(dir, { recursive: true, force: true })
  tmpDirs.length = 0
})

describe('Session Pudding Context Engine runtime integration', () => {
  ...
  it('does not expose legacy SaveMemory after Pudding Memory Review takes over', async () => {
    ...
    expect(defs).not.toContain('PuddingContextInspect')
    expect(defs).not.toContain('PuddingContextRefresh')
    expect(defs).toContain('PuddingMemorySearch')
  ...
  it('compacts conversation history without extracting legacy file-based memories', async () => {
    ...
    expect(existsSync(path.join('/tmp/pudding-session-context-config', 'projects'))).toBe(false)
  })
  ...
  it('injects a protocol-neutral context bundle before streaming and falls back when bundle generation fails', async () => {
    const session = await makeSession({
      provider: providerFromChunks([
        ...
        expect(text).toMatch(/<pudding-context-engine bundle="ctx_[0-9a-f]{16}">/)
    ...
    const fallbackSession = await makeSession({
      provider: providerFromChunks([
        ...
        expect(textFromSystemPrompt(config.systemPrompt)).not.toContain('<pudding-context-engine bundle=')
      }),
      contextConfig: { injectionEnabled: true, harvestEnabled: false },
  ...
  it('backfills previous assistant project summaries for confirmation harvest turns', async () => {
    ...
    const projectSummary = [
      '# 项目整体架构',
      'packages/core 负责 Pudding Context Engine、Session runLoop、harvest、工具注册和上下文注入。',
      'packages/electron 负责桌面主进程、IPC、窗口和系统服务。',
      'packages/ui 负责 React 聊天界面、Inspector、Context Panel 和设置界面。',
      '常用命令包括 pnpm --filter @puddingagent/core test 和 pnpm --filter @puddingagent/core build。',
    ].join('\n')
    const session = await makeSession({
    ...
    expect(candidateText).toContain('packages/core 负责 Pudding Context Engine')
...
describe('Sub-session Pudding Context Engine runtime integration', () => {
  it('injects configured context and enqueues harvest with the current sub-session model binding', async () => {
    ...
    const result = await runSubSession({
      ...
      provider: providerFromChunks([
        ...
        expect(textFromSystemPrompt(config.systemPrompt)).toMatch(/<pudding-context-engine bundle="ctx_[0-9a-f]{16}">/)
...
function makeTempDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), 'pudding-session-context-'))
  tmpDirs.push(dir)
  return dir
}
