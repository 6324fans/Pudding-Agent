...
3. Push with -u, create PR with gh pr create`,
  ...
  async execute(input: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
    ...
    const cwdFile = join(tmpdir(), `puddingagent-cwd-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`)
    ...
    if (isWindows) {
      ...
      const wrappedCommand = `${normalizedCommand}; __puddingagent_exit=$?; pwd -P > ${posixCwdFile} 2>/dev/null; exit $__puddingagent_exit`
      ...
      const wrappedCommand = `${rcPrelude}${command}; __puddingagent_exit=$?; pwd -P > ${cwdFile} 2>/dev/null; exit $__puddingagent_exit`
      shellCmd = userShell
