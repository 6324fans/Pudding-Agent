...
import { loadWebToolConfig } from '../config.js'
...
export const webFetchTool: ToolHandler = {
  ...
  async execute(input: Record<string, unknown>, context: ToolContext): Promise<ToolResult> {
    ...
    try {
      const fetchOptions = buildWebFetchOptions(loadWebToolConfig().webProxy, {
      ...
      return { content: `Error fetching URL: ${formatWebRequestError(err)}`, isError: true }
    }
  },
}
