...
export async function searchMemoryRecords(input: unknown = {}, options: MemorySearchOptions = {}): Promise<MemorySearchPayload> {
  ...
  if (!parsedInput.success) {
    return MemorySearchPayloadSchema.parse({ status: 'unavailable', searchedAt: now(), query: {}, results: [], diagnostics: [{ id: `diag_memory_search_${now()}`, level: 'warning', source: 'PuddingMemorySearch', message: `Memory search rejected invalid input: ${parsedInput.error.message}`, createdAt: now() }] })
  }
  ...
  try {
    ...
    return MemorySearchPayloadSchema.parse({ status: 'unavailable', searchedAt: now(), query: parsed, results: [], diagnostics: [{ id: `diag_memory_search_${now()}`, level: 'error', source: 'PuddingMemorySearch', message: error instanceof Error ? error.message : String(error), createdAt: now() }] })
  }
}
  ...
  if (!result.ok) {
    return {
      ...
      diagnostics: result.diagnostics.length
        ? result.diagnostics
        : [{ id: `diag_memory_search_evidence_${now()}`, level: 'warning', source: 'PuddingMemorySearch', message: 'Memory search could not load citation evidence; searching accepted fact text only.', createdAt: now() }],
    }
  }
