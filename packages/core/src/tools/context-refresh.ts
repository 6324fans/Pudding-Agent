...
export function createContextRefreshTool(options: RefreshContextOptions = {}): ToolHandler {
  return {
    definition: {
      name: 'PuddingContextRefresh',
      description: 'Refresh selected Pudding Context Engine providers and return an inspectable bundle with provider health and timing diagnostics.',
      inputSchema: {
        type: 'object',
        properties: {
...
function contextRefreshDiagnostic(message: string, level: ContextDiagnostic['level'], createdAt: number): ContextDiagnostic {
  return { id: `diag_context_refresh_${createdAt}`, level, source: 'PuddingContextRefresh', message, createdAt }
}
...
function createRefreshRequest(input: ContextRefreshInput, createdAt: number, healthOnly = false): ContextRequest {
  return {
    ...
    userMessage: input.userMessage ?? 'Refresh Pudding Context Engine providers.',
    recentMessages: [],
