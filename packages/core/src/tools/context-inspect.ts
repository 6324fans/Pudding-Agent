...
export async function inspectContext(input: InspectContextInput = {}, options: InspectContextOptions = {}): Promise<ContextInspectPayload> {
  ...
  if (options.inspectEnabled === false) return ContextInspectPayloadSchema.parse(emptyPayload('disabled', now(), [diagnostic('Pudding Context Engine inspection is disabled.', 'info', now)]))
...
export function createContextInspectTool(options: InspectContextOptions = {}): ToolHandler {
  return {
    definition: {
      name: 'PuddingContextInspect',
      description: 'Inspect the latest Pudding Context Engine injected bundle, citations, confidence, freshness, token cost, harvest queue, memory review, provider health, and Repo Wiki summary.',
...
function diagnostic(message: string, level: ContextDiagnostic['level'], now: () => number): ContextDiagnostic {
  return { id: `diag_context_inspect_${now()}_${Math.random().toString(36).slice(2)}`, level, source: 'PuddingContextInspect', message, createdAt: now() }
}
