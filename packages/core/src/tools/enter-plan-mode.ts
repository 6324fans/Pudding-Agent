...
export function isPlanModeToolAllowed(
  ...
  if (toolName === 'file_write') {
    ...
    const planDir = path.resolve(cwd, '.puddingagent', 'plans')
  ...
  return {
    definition: {
      ...
      description:
        ...
        'In plan mode you can read files, write plans to .puddingagent/plans/, and dispatch explore agents. ' +
    ...
    async execute(_input: Record<string, unknown>, _context: ToolContext): Promise<ToolResult> {
      onEnter()
      return {
        content: 'Plan mode activated. You can now read files, search code, and write your plan to .puddingagent/plans/. Call exit_plan_mode when ready for user review.',
      }
    },
  }
}
