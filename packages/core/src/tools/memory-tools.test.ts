...
describe('Pudding Context Engine memory tools', () => {
  it('describes durable project-memory search and write contracts to the model', () => {
    ...
    expect(writeTool.definition.description).toBe([
      'Write an accepted, citation-backed Pudding Context Engine memory fact into the current project store.',
      ...
      'Requires citations with id/type/ref, optional line/range/timestamp/hash, or a citation string shortcut. Data persists under <project>/.puddingagent/context-engine/context.db.',
    ].join(' '))
    expect(searchTool.definition.description).toBe([
      'Search accepted durable Pudding Context Engine memory facts from the current project store (project/repo/global only).',
  ...
  it('searches accepted durable project memory by query, scope, kind, and confidence', async () => {
    const store = makeStore({ facts: [fact({ id: 'memory_fact', content: 'Use test-first changes for Pudding Context Engine tools.' })] })
  ...
  it('applies memory search limit after query, scope, and kind filters', async () => {
    const store = makeStore({
      facts: [
        ...
        fact({ id: 'older_matching', content: 'Use test-first changes for Pudding Context Engine tools.', updatedAt: 10 }),
      ],
    })
    ...
    const store = makeStore({
      facts: [
        ...
        fact({
          ...
          content: 'PUDDINGAGENT 发布流程：bump version，commit，tag，然后 push tag 触发 release workflow。',
          updatedAt: 1,
        }),
      ],
    })
  ...
  it('finds a written memory by the original citation evidence even when fact content does not repeat the query', async () => {
    const cwd = mkdtempSync(path.join(os.tmpdir(), 'pudding-memory-search-'))
    try {
      const store = await openContextStore({ cwd, now: () => 10_000 })
      const write = await writeMemoryRecord({
        kind: 'architecture_decision',
        content: 'Pudding Context Engine uses a per-process singleton and registers code tools through context-engine-tools.ts.',
        citation: '用户问：PuddingMemorySearch 这个不是说写一条项目记忆然后立刻搜出来验证吗？',
        confidence: 0.95,
      }, { store, cwd, now: () => 10_000 })
      expect(write.status).toBe('accepted')

      const search = await searchMemoryRecords({ query: 'PuddingMemorySearch', limit: 5 }, { store, cwd, now: () => 10_001 })
