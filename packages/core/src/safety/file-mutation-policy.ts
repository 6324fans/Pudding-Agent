...
const MUTATION_TOOLS = new Set(['file_edit', 'multi_edit', 'file_write'])
...
export function evaluateFileMutationPolicy(policyInput: FileMutationPolicyInput): FileMutationPolicyDecision {
  ...
  if (policyInput.toolName === 'file_write' && !existsSync(resolvedFilePath)) {
    return allow()
  }

  if (policyInput.toolName === 'file_edit') {
    return checkFreshRead(policyInput.fileReadState, resolvedFilePath, getRequiredText(policyInput.input, 'old_string'))
  }

  if (policyInput.toolName === 'multi_edit') {
  ...
  if (policyInput.toolName === 'file_write') {
