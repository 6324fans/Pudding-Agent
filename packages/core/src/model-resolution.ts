...
export function resolveConfiguredModel(
  groups: ConfiguredModelGroup[] | undefined,
  request: string,
  ...
  const byEntryId = collect(groups, model => model.id === requested)
  if (byEntryId.length === 1) return { status: 'resolved', model: byEntryId[0] }
  if (byEntryId.length > 1) return ambiguous(requested, byEntryId)
...
function resolveComposite(groups: ConfiguredModelGroup[], requested: string): ConfiguredModelResolution | null {
  ...
  if (!group) {
    return { status: 'not_found', message: `Configured model group "${groupId}" was not found for "${requested}".`, matches: [] }
  }
  const model = group.models?.find(m => m.id === modelId || m.modelId === modelId || m.name === modelId)
  if (!model) {
    return { status: 'not_found', message: `Configured model "${modelId}" was not found in group "${groupId}".`, matches: [] }
  }
...
function collect(
  ...
  predicate: (model: ConfiguredModelEntry, group: ConfiguredModelGroup) => boolean,
