...
export class ConversationHistory {
  ...
  private getLastMessageSummary(sessionId: string): { role: string; preview: string; timestamp: number } | null {
    const stmt = this.db!.prepare("SELECT role, content, timestamp FROM messages WHERE session_id = ? AND role = 'user' ORDER BY timestamp DESC")
    stmt.bind([sessionId])
    let result: { role: string; preview: string; timestamp: number } | null = null
    while (stmt.step()) {
      const row = stmt.getAsObject()
      try {
        const blocks = JSON.parse(row.content as string)
        if (!Array.isArray(blocks) || blocks.every((b: any) => b.type === 'tool_result')) continue
        const text = blocks
        ...
        result = {
          ...
          preview: text || (hasImage ? '[图片]' : hasToolUse ? '[工具调用]' : ''),
  ...
  listSessions(cwd?: string): Array<{ id: string; projectName: string; cwd: string; title: string | null; createdAt: number; updatedAt: number; lastMessagePreview?: string | null; lastMessageRole?: string | null; lastMessageAt?: number | null }> {
    const stmt = cwd
    ...
    while (stmt.step()) {
      const row = stmt.getAsObject()
      const lastMessage = this.getLastMessageSummary(row.id as string)
      results.push({
  ...
  searchSessions(query: string, cwd?: string): SessionSearchResult[] {
    ...
    for (const session of sessions) {
      ...
      for (const message of messages) {
        const searchableText = this.messageToSearchText(message)
        ...
        if (matches.length < 3) {
          matches.push({
            ...
            snippet: this.makeSnippet(searchableText, normalizedText.indexOf(normalizedQuery), query.trim().length),
          })
        }
      }
  ...
  private messageToSearchText(message: Message): string {
  ...
  private makeSnippet(text: string, index: number, queryLength: number): string {
    const radius = 72
