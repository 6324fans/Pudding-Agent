...
import type { ContextEngineConfig, HarvestCandidate, HarvestModelBinding, ProviderProtocol } from './context/types.js'
    ...
    for (const tu of toolUses) {
      ...
      if (opts.agentType === 'plan' && tu.name === 'file_write') {
        ...
        if (!isWriteAllowedForPlanAgent(writePath, cwd)) {
          const restrictResult = { content: 'Plan agent can only write to .puddingagent/plans/ directory', isError: true }
...
function removeContextPromptSegments(systemPrompt: ModelConfig['systemPrompt']): ModelConfig['systemPrompt'] {
  ...
  return systemPrompt.filter(segment => !(segment as { puddingContextEngine?: boolean }).puddingContextEngine)
}
