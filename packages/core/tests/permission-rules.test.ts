...
describe('loadPermissionRules', () => {
  const tmpDir = path.join(os.tmpdir(), 'puddingagent-perm-test-' + Date.now())
  const projectDir = path.join(tmpDir, 'project')
  const globalDir = path.join(tmpDir, 'global')

  beforeEach(() => {
    mkdirSync(path.join(projectDir, '.puddingagent'), { recursive: true })
  ...
  it('should load project and global rules from JSON files', () => {
    ...
    writeFileSync(
      path.join(projectDir, '.puddingagent', 'permissions.json'),
