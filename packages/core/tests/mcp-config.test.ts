...
const TEST_DIR = path.join(os.tmpdir(), 'puddingagent-mcp-test-' + Date.now())
const TEST_CWD = path.join(TEST_DIR, 'project')

beforeEach(() => {
  mkdirSync(path.join(TEST_CWD, '.puddingagent'), { recursive: true })
})
...
describe('loadMcpConfig', () => {
  ...
  it('merges project config over global', () => {
    ...
    const projectPath = path.join(TEST_CWD, '.puddingagent', 'mcp-servers.json')
