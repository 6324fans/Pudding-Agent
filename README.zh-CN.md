...
Pudding-Agent 是一个桌面端 AI 编程助手，连接你的模型，通过 **30+ 内置工具**让 AI 完整操控你的代码库。它支持 Claude、GPT、Gemini、Ollama 以及任何 OpenAI 兼容接口 — 原生支持三种协议（`Anthropic Messages`、`OpenAI Chat Completions`、`OpenAI Responses`）。
...
Pudding-Agent 不自带模型 — 你需要自己的 API Key。打开**设置**（`⌘,` / `Ctrl+,`），进入**「模型」**选项卡。
...
Skills 是可复用的提示词模板，自动变成斜杠命令。在 `.puddingagent/skills/` 下放一个 markdown 文件即可自动发现。

**示例** — 创建 `.puddingagent/skills/code-review.md`：
...
Skills 可放在 `~/.puddingagent/skills/`（全局）或 `<项目>/.puddingagent/skills/`（项目级）。项目级覆盖同名的全局技能。

### 🎯 规划模式

AI 进入受限只读模式分析代码并撰写实施方案，你审阅通过后才执行修改。

按 `Shift+Tab` 或点击编辑器工具栏的规划开关。方案以 markdown 形式保存在 `.puddingagent/plans/`。

### 🔌 MCP 服务器

通过 [Model Context Protocol](https://modelcontextprotocol.io) 接入外部工具。在 `.puddingagent/mcp-servers.json`（项目）或 `~/.puddingagent/mcp-servers.json`（全局）中配置。
...
在工具调用前后执行 shell 命令，实施策略检查或触发副作用。在 `.puddingagent/hooks.json` 中配置。
...
AI 记住你的偏好、项目背景和历史决策。压缩上下文时自动提取记忆，以 markdown 文件存储在 `~/.puddingagent/projects/<项目路径>/memory/`。
...
**`PUDDINGAGENT.md`** — 放在项目根目录提供项目级指引，或放在 `~/.puddingagent/PUDDINGAGENT.md` 作为全局指令。内容注入系统提示词。

**`.puddingagent/rules/*.md`** — 模块化规则文件，与 `PUDDINGAGENT.md` 一同加载。用于分离关注点（如 `testing.md`、`style.md`、`deployment.md`）。
...
所有配置文件位于项目下的 `.puddingagent/`（项目范围）或 `~/.puddingagent/`（全局范围）。项目配置覆盖全局配置。

```
.puddingagent/
├── PUDDINGAGENT.md          # 项目指令（注入系统提示词）
...
Pudding-Agent 是源码可见软件，仅允许非商业使用，禁止商业用途。详见 [LICENSE](./LICENSE)。
